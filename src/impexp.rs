...
fn export_txs_from_block(txs: Vec<Hash>, client: &mut Client, file: &mut File) {
    txs.iter().for_each(|htx| {
        ...
        let buf = serialize::rmp_serialize(&tx).expect("Error during serialization of transaction");
        file.write(&buf).expect("Unable to write file...");
        //println!("TX: {:?}",buf);
    });
}
...
pub fn import_txs(file_name: &str, _client: &mut Client) {
    let bin = std::fs::read(file_name).expect("Unable to read import file");
    //let transactions:Vec<Transaction> = serialize::rmp_deserialize(&bin).unwrap();
    let transactions: Vec<Transaction> = serialize::rmp_deserialize(&bin).expect("Unable to deserialize input transactions");
